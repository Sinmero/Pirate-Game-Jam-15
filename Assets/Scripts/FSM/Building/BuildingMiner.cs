...
public class BuildingMiner : BuildActive
{
    ...
    public override void Execute()
    {
        ...
        {
            if (_buildingController._minerHologram._resourceVein == null) // if there is no resource vein nearby
            {
                AudioManager.instance.PlaySoundClip(AudioManager.instance._buzz);
                return;
            }
            if (_buildingController.BuildFloorCheck() && _buildingController.BuildOverlapCheck(_buildingController._minerHologram))
            { //check if the space we are building on is valid
